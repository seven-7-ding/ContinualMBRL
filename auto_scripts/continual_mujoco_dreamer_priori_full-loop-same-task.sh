...
CUDA_DEVICES=(6 7 6 7 6 7)

# Maximum concurrent runs launched by this script on each GPU.
MAX_RUNS_PER_GPU=1

# Task string (same for all settings)
TASK_STRING="quadruped_walk|quadruped_run|quadruped_escape|quadruped_fetch"

# Prefix for log directories
PREFIX="continual_dreamer_full_loop_same_task"
...
# Quadruped prior-observation/action dimensions.
QUADRUPED_OBS_DIM=78
...
for setting_spec in "${SETTINGS[@]}"; do
    ...
    cmd_args=(
        ...
        --env.continual_dmc_priori.obs_dim "$QUADRUPED_OBS_DIM"
        --env.continual_dmc_priori.task_action_space $QUADRUPED_ACT_DIM
    )
