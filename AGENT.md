...
- 2026-08-13 21:29 HKT update: user requested pausing the 6 current size1m walker/hopper/fish jobs in groups `no_wsc` and `wsc_skip_last_layer_init_all`. Sent `SIGSTOP` to main PIDs `3684824/3684825/3684826` and `3623047/3620503/3621821` plus their `wandb-core`/`wandb-xpu` descendants. Backup and resume artifacts are in `logdir/continual_dreamer_soft_reset_size1m/_pause_backup_20260813_212908_HKT`. Use `resume_live.sh` to `SIGCONT` the paused processes in place; use `restart_from_checkpoint.sh` only if the old PIDs are gone.
- 2026-08-13 21:47 HKT update: user clarified CUDA must be released. Terminated the 6 paused main training PIDs and their W&B descendants, verified target PIDs no longer exist and no longer appear in `nvidia-smi`. Also terminated the 6 related `wandb_metrics_proxy.py` sidecars after saving their PID/cmdline records under the same backup directory. Future resume should use `restart_from_checkpoint.sh`, not `resume_live.sh`, because the original PIDs are gone.
- 2026-08-13 22:04 HKT update: repaired missing baseline vision `data_augmentation_batch_align/seed_3000` training process after it had exited with GPU OOM at 21:33. Relaunched from `/home/jiale/MBRL/ContinualMBRL-baseline` with conda activation, `CUDA_VISIBLE_DEVICES=0`, `--egl_device 0`, original W&B run id `baseline-dmc-vision-size12m-da-align-3000-dmc-vision-size12m-da-align-1m-x5-v1`, and original logdir. New PID is `3862426`; it compiled, resumed W&B, loaded checkpoint `20260813T213207F639757`, and reached `Start training loop`.
- 2026-08-15 02:06 HKT update: implemented `wsc_skip_last_layer_constantinit`, parsed as `norm_mode=constantinit`. It behaves like `wsc_skip_last_layer_dout` for RMSNorm-followed target layers but uses each layer's initialization FNorm divided by 8 as the target norm. Added unit coverage in `embodied/tests/test_wsc.py`; `python -m pytest embodied/tests/test_wsc.py -q` passed with 8 tests.
- 2026-08-15 02:06 HKT update: launched size1m walker/hopper/fish runs under `logdir/continual_dreamer_soft_reset_size1m/wsc_skip_last_layer_constantinit_all/seed_{1000,2000,3000}`. Active main PIDs at validation: seed1000 `4036643` on GPU0, seed2000 `4036920` on GPU6, seed3000 `4037194` on GPU2. Validation reached valid `metrics.jsonl` logging counts seed1000 `5/5` at step 50000, seed2000 `7/5` at step 70000, seed3000 `5/5` at step 50000; all three remained alive. Metrics include finite `opt/wsc/target_norm/*` keys for controlled layers.
- 2026-08-16 16:51 HKT update: user requested stopping all live tasks for W&B project `continual_dreamer_soft_reset_walker_run|hopper_hop|fish_swim_vision_size12m_1m_x5` and freeing CPU/CUDA. Created cross-repo pause/resume backup at `/home/jiale/MBRL/vision_project_pause_backup_20260816T165045_HKT`, covering 12 main Dreamer training PIDs, 9 `wandb_metrics_proxy.py` sidecars, and 45 descendants. Terminated only those target PIDs; verification found no target PID alive, no exact-project process alive, and no target PID in `nvidia-smi`. Other size1m experiments, including `wsc_skip_last_layer_constantinit_all`, were left running.
- 2026-08-16 17:22 HKT update: started foreground supervisor `tools/launch_walker_hopper_cheetah_size1m_supervisor.py` for new project `continual_dreamer_soft_reset_walker_run|hopper_hop|cheetah_run_vision_size1m_1m_x5`. It manages 12 jobs: baseline `no_wsc`, WSC `wsc_skip_last_layer_constantinit_all`, baseline `l2_init_2e-4`, and baseline `data_augmentation_batch_align`, each with seeds `1000/2000/3000`. It uses `dmc_vision continual_dmc_vision size1m`, task `walker_run|hopper_hop|cheetah_run`, `task_interval=1000000`, `task_repeat=5`, `steps=15000000`, train ratio `256`, and activation/gradient ReDo logging `log+erank+srank`. Supervisor state/logs are under `logdir/continual_dreamer_soft_reset_walker_run|hopper_hop|cheetah_run_vision_size1m_1m_x5/_supervisor/`.
- 2026-08-16 17:22 HKT update: RAM guard is `MemAvailable >= 30GiB`; if violated, the supervisor stops managed jobs from low to high priority: `data_augmentation_batch_align`, `l2_init_2e-4`, `wsc_skip_last_layer_constantinit_all`, then `no_wsc`. It only manages PIDs it launched or matching its exact logdirs and will resume failed jobs when RAM and CUDA memory are available. First baseline launch attempt failed because baseline lacks `--run.log_policy_video`; the supervisor was patched to omit that flag and relaunched baseline jobs successfully to initialization/compile.
- 2026-08-16 17:36 HKT update: all 12 walker/hopper/cheetah vision size1m jobs wrote valid `metrics.jsonl` rows at step `10000` while the foreground supervisor remained active. FPS range was about `15.49-26.04`. Per-row key counts confirmed mechanism logging: data augmentation `592`, no_wsc `588`, l2_init `764`, WSC constantinit `883`.
